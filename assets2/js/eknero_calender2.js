		...
		for(var list_day = 1; list_day <= days_in_month; list_day++){
		 ...
		 if((eventday in events)){
			  ...
			  for(var izn = 0; izn < events[eventday].length; izn++){
				  ...
				  div_row += '<tr id="row_' + data_n['id'] + '"><td>' + data_n['begin_time'] + '</td><td>' + data_n['cname'] + '</td><td>' + ((data_n['cur_limit'] >= data_n['nlimit'])? 'Filled' : 'Available') + '</td><td>' + data_n['cost'] + '</td><td>' + ((data_n['ctime'] == 1)? ((data_n['cur_limit'] <= data_n['nlimit'])? '<button onClick="reserve_class(' + data_n['id'] + ')" style="float:right;" class="btn btn-primary">Reserve</button>' : '') : 'Expired/Finished') + '</td></tr>';
