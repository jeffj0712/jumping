...
if(isset($_POST['usid'])){
	...
	$time_stamp_end = mktime(23, 59, 59, $month, $lastday, $year);
		...
		$pdo =  new mypdo;
	$event_m = $pdo->get_events($usid, $time_stamp_start, $time_stamp_end);
...
<html class=" " lang="en">
	<head>
		<title>Jumping Fitness Malaysia | Class Calender</title>
		...
		.calender_bd{}
		.calender_bd > div{position:relative; display:inline-block; width: 14%; margin: 2px 1px 2px 1px; padding:5px 2px 5px 2px;  text-align:left;  border-radius:3px; background-color:#CCC; height:100px; overflow:auto}
		.number_day{position:absolute; right:2px; top:0px; font-weight:bold}
		.div_show{ font-size:11px; font-weight:bold; color:#030; border-bottom:thin #009 solid; padding-bottom:2px;}
		@media (max-width: 760px) {
		...
		<div class="right_col" role="main" style="min-height: 1154px;">
			<div class="">
				<div style="text-align:right"><a class="btn btn-primary fa fa-plus" href="add-class.php"> New Class</a><a class="btn btn-primary fa fa-mail-reply" href="class.php"> Back to List View</a></div>
				<div class="">
					
					<h4 style="size:40px; color:#21991E; font-weight:bold"><i class=" fa fa-bicycl"></i> Class Calender </h4>
					
				</div>
				
				<div style="width:100%; overflow-x:auto; border-top:solid 2px #03C">
			...
			<script src="../assets2/js/eknero_calender.js"></script>
			...
			public function get_events($uid, $time_st, $time_end){
					
					$qry = "SELECT id, cname, begin_date, begin_time, time, nlimit, cur_limit, cost FROM class WHERE (time >= ? AND time <= ?) ORDER BY time DESC";
					$stmt = $this->pdc->prepare($qry);
					$stmt->bindParam(1,  $time_st, PDO::PARAM_INT);
