...
<html class=" " lang="en">
	<head>
		<title>Jumping Fitness Malaysia | Admin</title>
		...
		<div class="right_col" role="main" style="min-height: 1154px;">
			<div class="">
				<div class="">
					...
					<h4 style="size:40px; color:#21991E; font-weight:bold"><i class=" fa fa-dashboard"></i>  Dashboard </h4>
...
for ($iz; $iz < count($rowz); $iz++) {
	...
	?>
					...
					<div class="col-xs-12" style="margin-top:30px; box-shadow: 2px 4px 6px #999; padding-bottom:30px;">
						<h3>Recent Cancelled Reservation</h3>
						<div class="table-responsive">
							<table cellpadding="5" class="table  table-striped table-bordered table-hover" style="background-color:#FFF">
								<thead class="bg-light">
									<tr class="border-0">
								...
								<tbody>
									...
									<?php
$rowz = $pdo->get_reservation_log(10);
$iz = 0;
for ($iz; $iz < count($rowz); $iz++) {
	$row = $rowz[$iz];
...
class mypdo {
	...
	public function get_reservation($lmt) {
		try {
			$qry = "SELECT a.id, b.cname, b.begin_date, b.begin_time, b.time, a.amount, c.fname, c.email, c.phone FROM reservation a INNER JOIN class b ON a.class_id = b.id INNER join profile c ON a.member_id = c.id  ORDER BY id DESC LIMIT ?";
			$stmt = $this->pdc->prepare($qry);
	...
	public function get_reservation_log($lmt) {
		try {
			$qry = "SELECT a.id, b.cname, b.begin_date, b.begin_time, b.time, a.amount, c.fname, c.email, c.phone FROM reservation_log a INNER JOIN class b ON a.class_id = b.id INNER join profile c ON a.member_id = c.id  ORDER BY id DESC LIMIT ?";
			$stmt = $this->pdc->prepare($qry);
			$stmt->bindParam(1, $lmt, PDO::PARAM_INT);
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
