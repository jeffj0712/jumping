...
<title>Jumping Fitness Malaysia | Cancelled Reservation Log</title>
...
public function get_reservation($uid, $lmt){
		 try{
		 $qry = "SELECT a.id, b.cname, b.begin_date, b.begin_time, b.time, a.amount FROM reservation_log a INNER JOIN class b ON a.class_id = b.id WHERE a.member_id = ?  ORDER BY id DESC LIMIT ?, 20";
		 $stmt = $this->pdc->prepare($qry);
		 $stmt->bindParam(1, $uid, PDO::PARAM_INT);
		 $stmt->bindParam(2, $lmt, PDO::PARAM_INT);
		 $stmt->execute();
